...
%}
%%
function[h,L,means,medians,bw]=violin(Y,varargin)
...
if isempty(find(strcmp(varargin,'facealpha')))==0
    facealpha = varargin{find(strcmp(varargin,'facealpha'))+1};
end
...
if size(facecolor,1)==1
    facecolor=repmat(facecolor,size(Y,2),1);
end
...
for i=1:size(Y,2)
    ...
    if isempty(b)==0
        [density, computedPoints, bandwidth] = ksdensity(Y{i},'bandwidth',b(i));
    elseif isempty(b)
        [density, computedPoints, bandwidth] = ksdensity(Y{i});
    end
    
    density = density / max(density) * 0.3; % normalize
    densities(:,i) = density;
    points(:,i) = computedPoints;
    medians(:,i) = nanmedian(Y{i});
    means(:,i) = nanmean(Y{i});
    bw(:,i) = bandwidth;
...
for i=i:size(Y,2)
    if isempty(lc) == 1
        if setX == 0
            h(i) = fill( [densities(:, i) + i; flipud(i - densities(:, i))], ...
                         [points(:, i); flipud(points(:, i))], ...
                          facecolor(i, :), 'FaceAlpha', facealpha, 'EdgeColor', 'none');
        else
            h(i)=fill([densities(:,i)+x(i);flipud(x(i)-densities(:,i))],[points(:,i);flipud(points(:,i))],facecolor(i,:),'FaceAlpha',facealpha,'EdgeColor','none');
        end
    else
        if setX == 0
            h(i)=fill([densities(:,i)+i;flipud(i-densities(:,i))],[points(:,i);flipud(points(:,i))],facecolor(i,:),'FaceAlpha',facealpha,'EdgeColor',lc);
        else
            h(i)=fill([densities(:,i)+x(i);flipud(x(i)-densities(:,i))],[points(:,i);flipud(points(:,i))],facecolor(i,:),'FaceAlpha',facealpha,'EdgeColor',lc);
        end
    end
    hold on
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(points(:,i),densities(:,i)+i,means(:,i)), interp1(flipud(points(:,i)),flipud(i-densities(:,i)),means(:,i)) ],[means(:,i) means(:,i)],mc,'LineWidth',2);
        end
        if plotmedian == 1
            p(2)=plot([interp1(points(:,i),densities(:,i)+i,medians(:,i)), interp1(flipud(points(:,i)),flipud(i-densities(:,i)),medians(:,i)) ],[medians(:,i) medians(:,i)],medc,'LineWidth',2);
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(points(:,i),densities(:,i)+i,means(:,i))+x(i)-i, interp1(flipud(points(:,i)),flipud(i-densities(:,i)),means(:,i))+x(i)-i],[means(:,i) means(:,i)],mc,'LineWidth',2);
        end
        if plotmedian == 1
            p(2)=plot([interp1(points(:,i),densities(:,i)+i,medians(:,i))+x(i)-i, interp1(flipud(points(:,i)),flipud(i-densities(:,i)),medians(:,i))+x(i)-i],[medians(:,i) medians(:,i)],medc,'LineWidth',2);
        end
    end
end
...
%% Set axis
if setX == 0
    axis([0.5 size(Y,2)+0.5, min(points(:)) max(points(:))]);
elseif setX == 1
    axis([min(x)-0.05*range(x) max(x)+0.05*range(x), min(points(:)) max(points(:))]);
end
